...
describe('GamePlayer index', function () {
    ...
    it('should display the Status bar', () => {
        ...
        expect($('.AdyenGame-status').isPresent()).to.eventually.equal(true);
    });

    it('should show at least one buyer', () => {

        browser.sleep(2000);

        expect($('.Buyer').isPresent()).to.eventually.equal(true);
    });
    ...
    it('should show a mid-game state', () => {
        ...
        Array.from({ length: 25 }).forEach(nextStateButton.click);
    ...
    it('should show the score in the end', () => {
        ...
        expect($('.AdyenGame-overlay').isPresent()).to.eventually.equal(true);
    });
});
